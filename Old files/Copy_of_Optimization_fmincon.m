...
%     ceq(15) = x(7) - (x(6) - 2*x(26)); %a1 = b1 - 2*t
...
%    
%     ceq(7) = x(14) - 1.64; % csd = 1.9
%     ceq(8) = x(15) - 2.09; % cld = 2.1
...
%Wrapper function for efficiency
function Eff2 = findEff(x)
%     x(7) = x(6) - 2*x(26); %a1 = b1 - 2*t
    x(11) = x(10) + x(27); % rhoS = rhoL + delta_rho
    x(13) = (x(25)*pi*x(5)*( (x(7) / 2)^2 ))/(1/x(11)); % mPCM = (f*pi*engine.L1*( (engine.a1 / 2)^2 ))/(1/engine.rhoS)
    Eff2 = findEfficiency(x);
end
