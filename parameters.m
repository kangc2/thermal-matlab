...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Parameters to change: a new structure
% includes all the parameters we want to test via structures
% Field name, lowerbound, upperbound, original value
input_range.length = [0.3, 1.3, engine.L1];
input_range.thickness = [0.08, 0.1, engine.b1];
input_range.latentheat = [0.49, 0.69, engine.Lh];
input_range.meltingTemp = [18, 37, engine.Tm];
input_range.rhoS = [600, 1000, engine.rhoS];
input_range.rhoL = [700, 1100, engine.rhoL];
...
upperbound = [1.3, 0.1, 0.69, 37, 1000, 900, 5];
% L1, b1, Lh, Tm, rhoS, rhoL, mPCM
...
% function F = findingP(P)
%     F = ((pi / 4)*(engine.L1*( ( (2*engine.a1) + ...
%             ( ( (P - Po)*engine.a1*(1 - engine.v^2) ) / engine.Ey)*( ( (engine.b1^2 + engine.a1^2) / (engine.b1^2 - engine.a1^2) ) +...
%         (engine.v / (1 - engine.v) ) ))*...
%             ( ( (P - Po)*engine.a1*(1 - engine.v^2) ) / engine.Ey)*( ( (engine.b1^2 + engine.a1^2) / (engine.b1^2 - engine.a1^2) ) + ...
%         (engine.v / (1 - engine.v) ) )) ))...
%     - ...
%     (( engine.mPCM*((1.3e-03 - (2.66e-04*log10( 1 + ( (P - Po) / 102.12) ) )) - engine.v1P) ) + ...
%     ( engine.mH*((engine.voH - (engine.CH*log10(1 + ( (P - Po) / engine.BH) ) )) - engine.voH) ) + (((engine.V1A*Po) / P) - engine.V1A));
% 
% end